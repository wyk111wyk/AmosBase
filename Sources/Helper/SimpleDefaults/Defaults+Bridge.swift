...
extension SimpleDefaults.CodableBridge {
...
/**
Any `Value` that conforms to `Codable` and `SimpleDefaults.Serializable` will use `CodableBridge` to do the serialization and deserialization.
*/
...
/**
`RawRepresentableCodableBridge` is needed because, for example, with `enum SomeEnum: String, Codable, SimpleDefaults.Serializable`, the compiler will be confused between `RawRepresentableBridge` and `TopLevelCodableBridge`.
*/
extension SimpleDefaults {
...
/**
We need both `SetBridge` and `SetAlgebraBridge` because `Set` conforms to `Sequence` but `SetAlgebra` does not. `Set` conforms to `Sequence`, so we can convert it into an array with `Array.init<S>(S)` and store it in the `UserDefaults`. But `SetAlgebra` does not, so it is hard to convert it into an array. Thats why we need the `SimpleDefaults.SetAlgebraSerializable` protocol to convert it into an array.
*/
extension SimpleDefaults {
...
extension SimpleDefaults {
	public struct AnyBridge: Bridge, Sendable {
		public typealias Value = SimpleDefaults.AnySerializable
		public typealias Serializable = Any
