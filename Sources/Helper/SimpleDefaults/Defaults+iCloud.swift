...
extension SimpleDefaults {
	...
	if shouldSync {
		SimpleDefaults.iCloud.add(.isUnicornMode)
	}
	```
	*/
	public enum iCloud {
		/**
		The singleton for SimpleDefaults's iCloudSynchronizer.
		*/
		...
		The synced keys.
		*/
		public static var keys: Set<SimpleDefaults.Keys> { synchronizer.keys }
		...
		*/
		// TODO: Support array of SimpleDefaults.Key after Swift 6 pack iteration is supported.
		// https://github.com/sindresorhus/Defaults/pull/185#discussion_r1704464183
		public static func add<each Value: SimpleDefaults.Serializable>(_ keys: repeat SimpleDefaults.Key<each Value>) {
		...
		*/
		public static func remove<each Value>(_ keys: repeat SimpleDefaults.Key<each Value>) {
		...
		Remove the keys that are set to be automatically synced.
		*/
		public static func remove(_ keys: [SimpleDefaults._AnyKey]) {
		...
		*/
		...
		Task {
			...
			print(SimpleDefaults[.isUnicornMode])
			//=> true

			await SimpleDefaults.iCloud.waitForSyncCompletion()
		...
		*/
		static func syncWithoutWaiting(_ keys: SimpleDefaults.Keys..., source: DataSource? = nil) {
...
extension SimpleDefaults.iCloud {
	/**
...
final class iCloudSynchronizer {
	...
	*/
	@Atomic(value: []) private(set) var keys: Set<SimpleDefaults.Keys>

	/**
	A thread-safe synchronization status monitor for `keys`.
	*/
	@Atomic(value: []) private var remoteSyncingKeys: Set<SimpleDefaults.Keys>

	// TODO: Replace it with async stream when Swift supports custom executors.
	private lazy var localKeysMonitor: SimpleDefaults.CompositeDefaultsObservation = .init { [weak self] pair, _ in
		guard
			let self,
	...
	*/
	func add<Value: SimpleDefaults.Serializable>(_ key: SimpleDefaults.Key<Value>) {
	...
	*/
	func remove<each Value>(_ keys: repeat SimpleDefaults.Key<each Value>) {
	...
	Remove the keys and stop the observation.
	*/
	func remove(_ keys: [SimpleDefaults._AnyKey]) {
	...
	*/
	...
	- Parameter keys: If the keys parameter is an empty array, the method will use the keys that were added to `SimpleDefaults.iCloud`.
	- Parameter source: Sync keys from which data source (remote or local).
	*/
	func syncWithoutWaiting(_ keys: [SimpleDefaults.Keys] = [], _ source: SimpleDefaults.iCloud.DataSource? = nil) {
	...
	*/
	private func syncKey(_ key: SimpleDefaults.Keys, source: SimpleDefaults.iCloud.DataSource) async {
	...
	*/
	private func syncFromRemote(forKey key: SimpleDefaults.Keys) async {
	...
	*/
	private func syncFromLocal(forKey key: SimpleDefaults.Keys) {
		guard
	...
	*/
	private func syncRemoteStorageOnChange() {
		if SimpleDefaults.iCloud.syncOnChange {
			synchronize()
		}
	}
	...
	*/
	private func timestamp(forKey key: SimpleDefaults.Keys, source: SimpleDefaults.iCloud.DataSource) -> Date? {
		switch source {
		case .remote:
			guard
	...
	*/
	func recordTimestamp(forKey key: SimpleDefaults.Keys, timestamp: Date?, source: SimpleDefaults.iCloud.DataSource) {
		switch source {
		case .remote:
			guard
				let values = remoteStorage.object(forKey: key.name) as? [Any],
	...
	*/
	private func latestDataSource(forKey key: SimpleDefaults.Keys) -> SimpleDefaults.iCloud.DataSource {
...
extension iCloudSynchronizer {
	...
	private static func logKeySyncStatus(
		_ key: SimpleDefaults.Keys,
		source: SimpleDefaults.iCloud.DataSource,
		syncStatus: SyncStatus,
		value: Any? = nil
	) {
		guard SimpleDefaults.iCloud.isDebug else {
			return
		}
	...
	private static func log(_ message: String) {
		...
		logger.debug("[SimpleDefaults.iCloud] \(message)")
	}
}
