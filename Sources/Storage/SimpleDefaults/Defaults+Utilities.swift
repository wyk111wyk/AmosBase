...
extension SimpleDefaults.Serializable {
	...
	```
	*/
	static func toValue<T: SimpleDefaults.Serializable>(_ anyObject: Any, type: T.Type = Self.self) -> T? {
		if
		...
		guard
			let nextType = T.Serializable.self as? any SimpleDefaults.Serializable.Type,
			nextType != T.self
		else {
			// This is a special case for the types which do not conform to `SimpleDefaults.Serializable` (for example, `Any`).
	...
	*/
	@usableFromInline
	static func toSerializable<T: SimpleDefaults.Serializable>(_ value: T) -> Any? {
		...
		guard let next = serialized as? any SimpleDefaults.Serializable else {
			// This is a special case for the types which do not conform to `SimpleDefaults.Serializable` (for example, `Any`).
			return serialized
		}
...
class Lock: SimpleDefaultsLockProtocol {
