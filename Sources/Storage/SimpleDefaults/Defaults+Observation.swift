...
extension SimpleDefaults {
	...
	Any `Defaults` key changes made within the closure will not propagate to `Defaults` event listeners (`SimpleDefaults.observe()` and `SimpleDefaults.publisher()`). This can be useful to prevent infinite recursion when you want to change a key in the callback listening to changes for the same key.

	- Note: This only works with `SimpleDefaults.observe()` and `SimpleDefaults.publisher()`. User-made KVO will not be affected.

	```swift
	let observer = SimpleDefaults.observe(keys: .key1, .key2) {
		// …

		Defaults.withoutPropagation {
			// Update `.key1` without propagating the change to listeners.
			SimpleDefaults[.key1] = 11
		}

		// This will be propagated.
		SimpleDefaults[.someKey] = true
	}
	```
	*/
	...
	*/
    final class DefaultsObservation: NSObject, @unchecked Sendable {
		...
		init(key: SimpleDefaults._AnyKey, _ callback: @escaping Callback) {
	...
	*/
	final class DefaultsObservationWithLifeTime: Observation {
		...
		init(key: SimpleDefaults._AnyKey, _ callback: @escaping DefaultsObservation.Callback) {
	...
	*/
	final class CompositeDefaultsObservation: Observation {
		...
		func add(key: SimpleDefaults._AnyKey, options: ObservationOptions = []) {
		...
		func remove(key: SimpleDefaults._AnyKey) {
	...
	let observer = SimpleDefaults.observe(.isUnicornMode) { change in
	...
	*/
	...
	let observer = SimpleDefaults.observe(keys: .setting1, .setting2) {
		// …
	}
	```
...
extension SimpleDefaults.ObservationOptions {
...
extension SimpleDefaults.KeyChange: Equatable where Value: Equatable {}
extension SimpleDefaults.KeyChange: Sendable where Value: Sendable {}
