...
public extension String {
    ...
    enum TestType: Identifiable, CaseIterable {
        case chineseAndEngish, chineseWuxia, chineseStory, chineseCasual, chinesePoem, englishSpeech, englishPoem, markdown01, markdown02, markdownCode, debugTest
    ...
    static func testText(_ type: TestType) -> String {
        ...
        case .debugTest:
        """
        "尊敬的主公，吾乃诸葛孔明，深知主公意欲领略现代编程之技。然吾以为200行代码若无明确之目的，则如沙地建塔，终将无用。主公可有具体需求？若无，容吾略陈思绪，编写一段简易而具启发之Swift代码，望能助主公开启一段编程之旅。\n\n```swift\n// 臣引以为例，拟写一个简易的计算器程序，或许能助主公此时之需。\n// 此Swift代码约共200行，具备加、减、乘、除四则运算以及简易的用户界面。\n\nimport Foundation\n\n// 定义操作符号枚举\nenum Operation {\n    case addition\n    case subtraction\n    case multiplication\n    case division\n 计算器类\nclass Calculator {\n    func performOperation(_ op: Operation, with operand1: Double, and operand2: Double) -> Double? {\n        switch op {\n        case .addition:\n            return operand1 + operand2\n        case .subtraction:\n            return operand1 - operand2\n        case .multiplication:\n            return operand1 * operand2\n        case .division:\n            guard operand2 != 0 else {\n                print(\"不可除以零\")\n                return nil\n            }\n            return operand1 / operand2\n        }\n    }\n}\n\n// 控制台用户界面\nclass ConsoleUI {\n    let calculator = Calculator()\n    \n    func start() {\n        var running = true\n        \n        while running {\n            print(\"请输入第一个数字：\")\n            guard let operand1 = readDouble() else {\n                continue\n            }\n            \n            print(\"请选择操作符 (+, -, *, /)：\")\n            guard let operation = readOperation() else {\n                continue\n            }\n            \n            print(\"请输入第二个数字：\")\n            guard let operand2 = readDouble() else {\n                continue\n            }\n            \n            if let result = calculator.performOperation(operation, with: operand1, and: operand2) {\n                print(\"结果为：\\(result)\")\n            }\n            \n            print(\"是否继续计算？(y/n)\")\n            if let continueInput = readLine(), continueInput.lowercased() != \"y\" {\n                running = false\n            }\n        }\n    }\n    \n    func readDouble() -> Double? {\n        if let input = readLine(), let value = Double(input) {\n            return value\n        } else {\n            print(\"输入无效，请输入一个数字。\")\n            return nil\n        }\n    }\n    \n    func readOperation() -> Operation? {\n        if let input = readLine() {\n            switch input {\n            case \"+\":\n                return .addition\n            case \"-\":\n                return .subtraction\n            case \"*\":\n                return .multiplication\n            case \"/\":\n                return .division\n            default:\n                print(\"无效的操作符，请输入 +, -, * 或 / 。\")\n                return nil\n            }\n        }\n        return nil\n    }\n}\n\n// 主程序入口\nlet consoleUI = ConsoleUI()\nconsoleUI.start()\n```\n\n主公，此简易计算器程序，虽朴素简约，但功能完备，可供主公一试。主公如有更具体的需求，吾将尽力为主公再作改进。"
        """
        }
    }
}
